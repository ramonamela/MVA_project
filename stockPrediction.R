...
1 - generateBenefitContinuousDay(Y.test, "PredLinearTest", filenames, amount_mean_lr)
...
for(i in 1:length(filenames)){
  ...
  for(j in 1:length(C)){
    ...
    for(z in 1:length(alpha)){
      ...
      model.svm<- svm(as.factor(Y.train[,pos]) ~ ., data=X.train.pca, type="C-classification", cost=C[j], gamma=alpha[z], kernel="radial", scale = FALSE)
...
for(i in 1:length(filenames)){
  print(paste0(i, ": ", filenames[i]))
  pos <- which(colnames(Y.train)==paste0('IncrementDayCategorical',filenames[i]))
  model.svm<- svm(as.factor(Y.train.val[,pos]) ~ ., data=X.train.val.pca, type="C-classification", cost=C[prevJSVM], gamma=alpha[prevZSVM], kernel="radial", scale = FALSE)
